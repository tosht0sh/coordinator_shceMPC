...
class MocapPoseReceiverNode(DTROS):
    """Receive mocap pose over UDP from coordinator laptop and publish it on /<vehicle>/mocap_reader."""

    def __init__(self, node_name):
        super(MocapPoseReceiverNode, self).__init__(node_name=node_name, node_type=NodeType.GENERIC)

        self._vehicle_name = os.environ["VEHICLE_NAME"]
        self._bind_ip = "0.0.0.0" # Bot IP
        self._port = 5005
        ...
        self._sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)    # Allow multiple listeners on the same port
        self._sock.bind((self._bind_ip, self._port))
        self._sock.setblocking(False)   # Use non-blocking socket to avoid stalling the ROS node

        self._pose_pub = rospy.Publisher(self._topic, Float64MultiArray, queue_size=30)
