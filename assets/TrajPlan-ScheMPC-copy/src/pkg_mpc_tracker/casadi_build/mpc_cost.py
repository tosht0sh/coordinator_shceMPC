...
# New cost_inside_ellipses

def cost_inside_ellipses_smooth(state: ca.SX, ellipse_param: list[ca.SX], weight: float = 1.0) -> ca.SX:
    indicator = inside_ellipses_smooth(state.T, ellipse_param)
    
    # Softplus approximation of fmax(0, x). 
    # beta=10 or 20 makes it follow the fmax shape closely but keeps it C-infinity smooth.
    beta = 10.0
    smooth_max = (1/beta) * ca.log(1 + ca.exp(beta * indicator))
    
    # Weight it and square it
    cost = weight * ca.sum1(smooth_max**2)
    return cost
